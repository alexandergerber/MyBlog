...
Here the function `purrr::pmap_dfr()` comes in handy. This function allows to iterate over multiple parameters in an extreamly concise way. We simply provide the entire parameter grid to `purrr::pmap_dfr()` which takes a row at a time and passes the values in the columns as arguments to the provided function. The additionally `dfr` tells us that a data.frame is returned created by row binding.    
